...
The engineering prediction of H_0^{Measure} . (M H_0)^{Excitor} ) for the voltage perturbation in a single excitor, measure coil configuration when an object is placed in an alternating low frequqncy magnetic field has been confirmed by a rigorous mathematical analysis [2].  This involves an asymptotic expansion of the perturbed magnetic field in the presence of a conducting object as it’s size tends to zero for the eddy current approximation of Maxwell’s equations. Here H_0^{Excitor} is the background field generated by an exiting coil evaluated at the centre of the object, H_0^{Measure} is the corresponding magnetic field if a sensor coil acts as an excitor and M is the MPT. The low-cost description provided by an MPT offers significant advantages for the location and characterisation of objects in metal detection. Characterising object’s by their MPT coefficients can viewed as an inverse solution where a-prori knowledge about the targets has been applied.
...
The provided program computes the MPT for a given object computed by solving 3 vectoral transmission problem simultaneously using hpfem [1,2]. This involves scaling the object and generating a mesh for the same shaped unit-sized conducting permeable object placed in a large non-conducting domain with the truncation domain set a large distance from the object. The domain is assumed to be discretised by an unstructured tetrahedral mesh. Reading meshes generated from Netgen are currently supported.
...
1. Save or clone the respository to your machine.
...
A detailed tutorial is in preperation, which will explain how to setup problem files and meshes for other objects.
