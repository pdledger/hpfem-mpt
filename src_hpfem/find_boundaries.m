...
function [iboun,bcosurf,nboun]=find_boundaries(npoin,nelem,ielem,mat,matc);
...
bcosurf(1)=2;%2; % Dirchlet Outer Boundary
           % 3 % Neumann Outer Boundary
bcosurf(2)=5; % Interface           
...
for i=1:nface
    if helpbcface1(i)==1
        ...
        iboun(nboun,5)=1; % fix 1 as the idendtifier of the exterior
    end
end
...
for i=1:nface
    if helpbcface1(i)==2
       ...
       % matc(helpbcface2(i,1)), matc(helpbcface2(i,2))
        if abs(matc(helpbcface2(i,1))-matc(helpbcface2(i,2)) )==1
